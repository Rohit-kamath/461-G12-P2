...
describe('RampUp Unit Tests', () => {
	...
	it('should correctly compute a ramp-up score based on mock data', async () => {
		...
		(RampUpAPI.fetchRepositoryContributors as jest.Mock).mockImplementation(() =>
			Promise.resolve([
				{
		...
		(RampUpAPI.fetchRepositoryStars as jest.Mock).mockImplementation(() =>
			Promise.resolve([
				{
					/*...mock star data...*/
				},
		...
		(RampUpAPI.fetchRepositoryForks as jest.Mock).mockImplementation(() =>
			Promise.resolve([
				{
					/*...mock fork data...*/
				},
