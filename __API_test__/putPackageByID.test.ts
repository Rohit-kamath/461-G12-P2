...
const logger = createModuleLogger('putPackageByID.test.ts');
logger.info("Starting tests for putPackageByID.test.ts");
describe('reset', () => {
...
describe('putPackageByID', () => {
    it('should return 404 status code to signifiy package not found', async () => {
        try {
            await axiosInstance.put(`/package/1`, {
                "metadata": {
                    "Name": 'test',
                    "Version": '1.0.0',
                    "ID": "1",
    ...
    it('should return 200 status code to signifiy successful put', async () => {
        try {
            const response = await axiosInstance.put(`/package/${metadata.ID}`, {
                "metadata": {
                    "Name": metadata.Name,
    ...
    it('GET /package/id onto the just updated package to see if it actually updated', async () => {
        try {
            const response = await axiosInstance.get(`/package/${metadata.ID}`);
            expect(response.data.data.Content).toBe(newContent);
        } catch (error: any) {
            console.log(error);
            console.log(error.response.status);
            throw error;
...
logger.info("Finished tests for putPackageByID.test.ts");